...
void Image::drawRectangle(unsigned int x,unsigned int y,unsigned int w,unsigned int h, Color color, bool fill) {
	...
	if (fill == true) {
		for (unsigned int x1 = x; x1 < (x + w); x1++) {
			for (unsigned int y1 = y; y1 < (y + h); y1++) {
		...
		for (unsigned int x1 = x; x1 < (x + w); x1++) {
			setPixel(x1, y, color);
			setPixel(x1, y + h - 1, color);
		}
		for (unsigned int y1 = y + 1; y1 < (y + h - 1); y1++) {
			setPixel(x, y1, color);
			setPixel(x + w -1, y1, color);
		}
	}
}
...
void Image::drawCircle(int x, int y, int r, Color color, bool fill) {
	...
	for ( int x1 = x - r; x1 < (x + diameter); x1++) {
		for ( int y1 = y - r; y1 < (y + diameter); y1++) {
